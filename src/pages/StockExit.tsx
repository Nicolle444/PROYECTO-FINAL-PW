...
export default function StockExit() {
  ...
  const resultingStock = product ? product.stock - quantity : 0
  const stockError = product && resultingStock < 0
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      await api.post('/api/v1/stock/exit', {
        productId: product.id,
        quantity,
        justification,
        userId: user?.id,
      })
      setSuccess(`Salida de ${quantity} unidades registrada para ${product.name}.`)
  ...
  return (
    <div className="max-w-lg mx-auto space-y-4">
      <h1 className="font-anton text-3xl text-primary tracking-wide">SALIDA DE STOCK</h1>
      ...
      {product && (
        <div className="bg-white rounded-xl shadow p-6">
          <form onSubmit={(e) => void handleSubmit(e)} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Cantidad a despachar *</label>
              <input
                type="number" min={1} value={quantity}
                onChange={(e) => setQuantity(Number(e.target.value))} required
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary"
              />
              <p className={`text-xs mt-1 font-medium ${stockError ? 'text-red-600' : 'text-gray-400'}`}>
                {stockError
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Ej: Despacho a cliente, orden de venta #456..."
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary"
              />
            </div>
            ...
            <button type="submit" disabled={saving || !!stockError} className="w-full bg-accent text-black font-semibold py-2.5 rounded-lg hover:bg-amber-400 transition-colors disabled:opacity-60 disabled:cursor-not-allowed">
