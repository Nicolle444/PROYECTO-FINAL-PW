...
export class RegisterEntryUseCase {
  ...
  readonly execute = async (input: RegisterEntryInput): Promise<StockMovement> => {
    ...
    const movement = new StockMovement({ id: uuidv4(), productId: input.productId, type: 'ENTRY', quantity: input.quantity, previousStock, newStock: product.stock, justification: input.justification || 'Stock entry', userId: input.userId, createdAt: new Date() })
